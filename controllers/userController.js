...
const multerFilter = (req, file, cb) => {
    if (file.mimetype.startsWith('image')) {
        ...
        cb(new AppError('uploaded file is not an image! Please upload only images.', 400), false)
    }
}
...
exports.resizeUserPhoto = catchAsync(async(req, res, next) => {
    ...
    await sharp(req.file.buffer).resize(1000, 1000).toFormat('jpeg').jpeg({ quality: 90 }).toFile(`starter/public/img/users/${req.file.filename}`);
